...
export function buildSupabaseSql(db, clientName) {
  ...
  for (const item of importedItems) {
    lines.push(
      `insert into billing_items (id, project_id, description, type, quantity, unit_price, amount, custom_amount, production_status, billing_status, delivered_at, delivered_by, invoice_id, note, created_at, created_by, updated_at, updated_by) values (${sql(item.id)}, ${sql(item.projectId)}, ${sql(item.description)}, ${sql(item.type)}, ${sql(item.quantity)}, ${sql(item.unitPrice)}, ${sql(item.amount)}, true, ${sql(item.productionStatus)}, ${sql(item.billingStatus)}, ${sql(item.deliveredAt)}, ${sql(item.deliveredBy)}, ${sql(item.invoiceId)}, ${sql(item.note)}, ${sql(item.createdAt)}, 'Import', ${sql(item.updatedAt)}, 'Import') on conflict (id) do nothing;`,
    );
  }
