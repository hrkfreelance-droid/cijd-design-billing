...
const REVIEW_CONFIG = "dist/server/wrangler.review.json";
...
if (branch !== BRANCH) fail(`review deploy is allowed only from ${BRANCH}; current=${branch}`);

const localHead = git("rev-parse", "HEAD");
const remoteHead = git("rev-parse", `refs/remotes/origin/${BRANCH}`);
if (localHead !== remoteHead) {
  fail(`local HEAD ${localHead} does not match origin/${BRANCH} ${remoteHead}`);
}
...
console.log(`[CIJD] build ${localHead.slice(0, 8)} for Review Worker`);
...
if (config.name !== REVIEW_WORKER || !config.name.endsWith("-preview")) {
  fail(`refusing to deploy worker ${String(config.name)}`);
}
...
console.log("[CIJD] Run npm run verify:live before claiming LIVE PASS.");
