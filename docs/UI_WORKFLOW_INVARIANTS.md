...
- Reuse DAISHIN's recent layout/interaction polish where it improves CIJD (mobile header density, 44px touch targets, iOS focus-zoom prevention, flatter nested surfaces, and tighter vertical rhythm), but do not copy DAISHIN typography. CIJD's existing English font stack and type feel are canonical for CIJD.
...
- Use the same rounded project container treatment for active Design and Printing queues. Item rows inside those containers stay flat and divider-led; do not add a second rounded card around each item.
