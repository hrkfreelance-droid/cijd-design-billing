...
export function PrintingWorkspace({ view }: { view: PrintingView }) {
  ...
  const knownTotal = roundMoney(items.reduce((total, item) => total + Math.max(item.amount ?? 0, 0), 0));
  const totalLabel =
    view === "history"
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0">
        ...
        <div data-testid="printing-item-financials" className="mt-1 flex min-w-0 flex-wrap gap-x-3 gap-y-0.5 text-[12px] text-muted">
          <span className="tnum whitespace-nowrap">
            {t("printing.cost")} {item.printCost != null ? money(item.printCost) : "—"}
          </span>
          <span className="tnum whitespace-nowrap font-medium text-default">
            {t(confirmed ? "printing.billingPrice" : "printing.suggestedBilling")} {shown != null && shown > 0 ? money(shown) : "—"}
          </span>
        </div>
        <div className="mt-1 flex min-w-0 flex-wrap gap-x-3 gap-y-0.5 text-[11.5px] text-faint">
          <span>
            {t("printing.priceConfirmedStatus")}: {confirmed ? t("printing.confirmed") : t("printing.waitingForPrice")}
          </span>
          <span>
            {t("printing.deliveryStatus")}: {finished ? t("delivery.done") : t("printing.waitingForDelivery")}
          </span>
          <span>
            {t("printing.billingHandoff")}: {item.billingStatus === "READY_TO_INVOICE" ? t("printing.billingReady") : item.billingStatus === "INVOICED" || item.billingStatus === "PAID" ? t("printing.billingInvoiced") : t("printing.waitingForDelivery")}
          </span>
        </div>
      </div>
      ...
      <div className="row-span-2 flex min-w-[84px] flex-col items-end gap-0.5">
        ...
        {(shown == null || shown <= 0) && <span className="whitespace-nowrap text-[11.5px] font-medium text-review">{t("printing.pricePending")}</span>}
        ...
        {!history && !locked && (
          !confirmed ? (
            ...
            <ItemProductionAction item={item} size="sm" deliveryLabel={t("printing.deliveryAction")} />
          )
        )}

        {!history && item.billingStatus === "READY_TO_INVOICE" && scope?.snapshot.scope.billing && (
          <Link href="/office" className="text-[11.5px] font-medium text-accent hover:underline">
            {t("printing.goBilling")}
          </Link>
        )}
...
function PrintEditSheet({
  ...
  const [printCost, setPrintCost] = useState(item.printCost == null ? "" : String(item.printCost));
  ...
  const validPrice =
    validQuantity && (validCost || (unitPriceValue !== null && unitPriceValue > 0 && calculatedAmount !== ""));

  const setPrice = async () => {
    if (
      !validQuantity ||
      (!validCost && (unitPriceValue === null || unitPriceValue <= 0 || calculatedAmount === ""))
    ) {
      return;
    }
    const ok = await run(
      async () => {
        await api(`/api/printing-items/${item.id}/spec`, {
          ...
          body: {
            ...
            ...(hasCost ? { printCost: costValue } : {}),
          },
        });
        await api(`/api/printing-items/${item.id}/price`, {
          method: "POST",
          body: {
            unitPrice: validCost ? calculatedBilling / quantityValue : unitPriceValue,
            amount: validCost ? calculatedBilling : Number(calculatedAmount),
            ...(hasCost ? { printCost: costValue } : {}),
            confirm: true,
          },
        });
  ...
  return (
    ...
    >
      <div className="space-y-4 pb-2">
        ...
        <Field label={t("printing.cost")} hint={t("printing.costHint")}>
          <Input
            inputMode="decimal"
            value={printCost}
            onChange={(event) => setPrintCost(event.target.value)}
            className="tnum"
            data-testid="printing-cost"
            placeholder="e.g. 20"
          />
        </Field>
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label={t("common.total")} hint={t("printing.autoCalculated")}>
            <Input
              value={hasCost ? calculatedBilling.toFixed(2) : calculatedAmount}
              readOnly
              aria-readonly="true"
