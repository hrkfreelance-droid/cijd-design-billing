...
export function PrintingWorkspace({ view }: { view: PrintingView }) {
  ...
  const items = useMemo(() => {
    ...
    return allItems.filter((item) => {
      ...
      return [project?.name, client?.name, item.description, item.printSize, item.note]
        .filter(Boolean)
        .join(" ")
        .toLowerCase()
        .includes(term);
  ...
  const totalCost = roundMoney(allItems.reduce((total, item) => total + Math.max(item.printCostAmount ?? 0, 0), 0));
  const pending = allItems.filter((item) => printPriceReviewState(item) !== "CONFIRMED").length;
  ...
  return (
    <div className="animate-rise mx-auto w-full max-w-[1000px] px-4 py-6 sm:px-6 sm:py-8">
      <header className="mb-5 flex flex-wrap items-end justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-[13px] text-muted">
            {allItems.length} {copy(locale, "件", "items")}
      ...
      <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-center">
        <div className="relative min-w-0 flex-1 sm:max-w-md">
          ...
          <input
            ...
            placeholder={copy(locale, "印刷案件を検索", "Search print jobs")}
            aria-label={copy(locale, "印刷案件を検索", "Search print jobs")}
      ...
      {allItems.length === 0 ? (
        ...
        <EmptyState title={copy(locale, "該当する印刷案件がありません", "No matching print jobs")} />
      ) : (
  ...
  return (
    <section className="overflow-hidden rounded-3xl border border-line bg-panel">
      <div className="flex items-start justify-between gap-4 px-5 py-4 sm:px-6">
        ...
        <Amount value={totalCost > 0 ? money(totalCost) : "—"} strong className="text-[18px]" />
      </div>
...
function PrintItemRow({ item, history, onOpen }: { item: BillingItem; history: boolean; onOpen: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      className="grid w-full grid-cols-[minmax(0,1fr)_auto] items-center gap-4 rounded-2xl px-3 py-3.5 text-left transition-colors hover:bg-fill/60"
    >
      <div className="min-w-0">
      ...
      <div className="flex shrink-0 items-center gap-3">
        <Amount value={cost > 0 ? money(cost) : "—"} strong className="text-[15px]" />
  ...
  return (
    ...
    >
      <div className="space-y-4 pb-2">
        ...
        {clients.length === 0 && (
          <p className="rounded-xl bg-fill px-3 py-2 text-[12px] text-muted">{copy(locale, "Admin の Clients から先にクライアントを登録してください", "Add a client from Admin > Clients first")}</p>
        )}
