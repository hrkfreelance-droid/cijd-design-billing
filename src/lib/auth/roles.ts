...
const MATRIX: Record<Role, Permission[]> = {
  ...
  BILLING: ["billing:read", "billing:price:write", "invoice:write", "payment:read", "progress:read"],
  ACCOUNTING: ["payment:read", "payment:write", "progress:read", "billing:price:write"],
  PRINTING: ["production:read", "printing:read", "print:write", "delivery:write", "billing:price:write"],
  ADMIN: [
    "production:read",
    "progress:read",
    "designer:read",
...
export function workspacesFor(role: Role): ("designer" | "printing" | "office")[] {
  ...
  if (canAny(role, ["billing:read", "billing:price:write", "payment:read", "progress:read"])) spaces.push("office");
  return spaces;
}
