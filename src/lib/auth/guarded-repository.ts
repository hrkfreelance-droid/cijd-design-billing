...
export class GuardedRepository {
  ...
  createServiceType(input: { name: string; actor?: string }) {
    this.assert("client:write");
    return this.repo.createServiceType({ ...input, actor: this.actor(input.actor) });
  }

  updateServiceType(id: string, patch: { name?: string; active?: boolean; actor?: string }) {
    this.assert("client:write");
    return this.repo.updateServiceType(id, { ...patch, actor: this.actor(patch.actor) });
  }
  ...
  setProjectBillingReadiness(
    ...
    return this.repo.setProjectBillingReadiness(id, readiness, this.actor(actor));
  }
  ...
  overrideBillingPrice(id: string, amount: number, actor?: string) {
  ...
  setBillingItemMarkup(id: string, markupPercent: number | null, actor?: string) {
    this.assert("billing:price:write");
    return this.repo.setBillingItemMarkup(id, markupPercent, this.actor(actor));
  }
  ...
  /**
   * Billing V2 finishes the work and records it in one step, so it is gated on
   * the same permission as issuing a bill. The database still has the final
   * say on each underlying change.
   */
  markProjectsBilled(input: Parameters<Repository["markProjectsBilled"]>[0]) {
    this.assert("invoice:write");
    return this.repo.markProjectsBilled({ ...input, actor: this.actor(input.actor) });
  }

  restoreProjectsToBilling(input: Parameters<Repository["restoreProjectsToBilling"]>[0]) {
    this.assert("invoice:write");
    return this.repo.restoreProjectsToBilling({ ...input, actor: this.actor(input.actor) });
  }

  deleteProject(id: string, actor?: string) {
    this.assert("production:write");
    return this.repo.deleteProject(id, this.actor(actor));
  }
