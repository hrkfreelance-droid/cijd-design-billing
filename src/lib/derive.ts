...
export function priceState(item: BillingItem): PriceState {
  if (item.type === "PRINT") {
    ...
    if (review === "REVIEW_REQUIRED") return item.amount != null && item.amount > 0 ? "SUGGESTED" : "PENDING";
    if (review === "CONFIRMED") return "CONFIRMED";
    return item.amount != null && item.amount > 0 ? "SUGGESTED" : "PENDING";
  }
  if (item.amount == null || item.amount <= 0) return "PENDING";
...
export function sum(items: { amount: number | null }[]): number {
  return Math.round(items.reduce((total, i) => total + (i.amount ?? 0), 0) * 100) / 100;
}
