...
export const toItem = (row: Row): BillingItem => ({
  ...
  discountType: (row.discount_type as BillingItem["discountType"]) ?? "NONE",
...
export const toInvoice = (row: Row): Invoice => ({
  ...
  showParentCompany: Boolean(row.show_parent_company),
  parentCompanyName: (row.parent_company_name as string) ?? null,
  pltFormat: (row.plt_format as Invoice["pltFormat"]) ?? "NORMAL",
  stateChargeVat: Boolean(row.state_charge_vat),
  noVat: Boolean(row.no_vat),
