...
export interface CreateBillingItemInput {
  ...
  description: string;
  type?: ItemType;
  /** Service registry key; see src/lib/billing-v2/services.ts. */
  serviceType?: string;
  quantity?: number;
  unitPrice?: number;
...
export interface MarkBilledInput {
  projectIds: string[];
  /** Defaults to today in the caller's timezone. */
  billedOn?: string;
  actor?: string;
}

export interface RestoreBilledInput {
  projectIds: string[];
  actor?: string;
}
...
export interface Repository {
  ...
  createServiceType(input: { name: string; actor?: string }): Promise<ServiceType>;
  updateServiceType(
    id: string,
    patch: { name?: string; active?: boolean; actor?: string },
  ): Promise<ServiceType>;
  ...
  /**
   * Billing V2's one state change: this work has been billed.
   *
   * Everything selected is finished, priced, and recorded on the ledger in a
   * single step, then leaves the Billing screen for Archive.
   */
  markProjectsBilled(input: MarkBilledInput): Promise<MarkBilledResult>;

  /** The same step, undone: billed work comes back to Billing, editable again. */
  restoreProjectsToBilling(input: RestoreBilledInput): Promise<MarkBilledResult>;

  /** Removes a project and everything on it. Billed work is restored first. */
  deleteProject(id: string, actor?: string): Promise<void>;
