    ...
    <div className="animate-rise">
      <PageHeader
        ...
        action={
          <>
            <PageTotal
              value={money(sum(inProgressItems.filter((item) => item.amount != null && item.amount > 0)))}
              label={estimated ? t("projects.estimatedTotal") : undefined}
              secondaryValue={
                scope.snapshot.exchangeRate
                  ? formatKhr(
                      sum(inProgressItems.filter((item) => item.amount != null && item.amount > 0)),
...
function ProjectTotal({ items }: { items: BillingItem[] }) {
  ...
  const knownTotal = sum(items.filter((item) => item.amount != null && item.amount > 0));
