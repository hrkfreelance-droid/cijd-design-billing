...
test("printing review confirms price before delivery and blocks unconfirmed invoices", async ({ page }) => {
  ...
  const readyDeliveryResponse = page.waitForResponse(
    (response) =>
...
test("printing cost persists as Billing price and keeps the handoff gates", async ({ page }) => {
  await signIn(page, "u_hiroki");
  const project = await (
    await page.request.post("/api/projects", {
      data: { clientId: "cl_ringer_hut", name: "Cost Billing Handoff Check" },
    })
  ).json();
  const print = await (
    await page.request.post("/api/billing-items", {
  ...
  await page.goto("/printing");
  const card = page.getByTestId("printing-item-card").filter({ hasText: "Cost Billing Handoff Check" });
  await card.getByRole("button", { name: "Set price" }).click();
  ...
  await dialog.getByRole("button", { name: "Set price" }).click();
  await priceResponse;
  ...
  const deliveryOnly = await (
    await page.request.post("/api/billing-items", {
      data: { projectId: project.data.id, description: "Delivery only", type: "PRINT", unitPrice: 0 },
    })
  ).json();
  const deliveryResponse = await page.request.post(`/api/billing-items/${deliveryOnly.data.id}/delivery`);
  expect(deliveryResponse.ok()).toBeTruthy();
  state = await (await page.request.get("/api/state")).json();
  expect(state.data.billingItems.find((item: { id: string }) => item.id === deliveryOnly.data.id)).toEqual(
    expect.objectContaining({ productionStatus: "DELIVERED", billingStatus: "NEEDS_REVIEW" }),
  );

  // The confirmed-but-undelivered item is also absent from Billing.
  await signIn(page, "u_billing");
  await page.goto("/office");
  await expect(page.getByText("Cost Billing Handoff Check", { exact: true })).toHaveCount(0);

  // Price Confirmed + Delivered is the only handoff that appears in Billing.
  await signIn(page, "u_hiroki");
  await page.goto("/printing");
  const readyCard = page.getByTestId("printing-item-card").filter({ hasText: "Cost Billing Handoff Check" });
  const readyDeliveryResponse = page.waitForResponse(
    (response) =>
      response.url().includes(`/api/billing-items/${print.data.id}/delivery`) &&
      response.request().method() === "POST" &&
  ...
  await readyCard.getByRole("button", { name: "Deliver" }).click();
  await page.getByRole("dialog").getByRole("button", { name: "Mark as delivered" }).click();
  await readyDeliveryResponse;
  state = await (await page.request.get("/api/state")).json();
  expect(state.data.billingItems.find((item: { id: string }) => item.id === print.data.id)).toEqual(
  ...
  await priceSheet.getByLabel("Billing price").fill("55");
  const overrideResponse = page.waitForResponse(
    (response) =>
      response.url().includes(`/api/billing-items/${print.data.id}/billing-price`) &&
      response.request().method() === "PATCH" &&
      response.status() === 200,
  );
  await priceSheet.getByRole("button", { name: "Save price" }).click();
  ...
  await expect(page).toHaveURL(/\/office\/payments$/);
  const locked = await page.request.patch(`/api/billing-items/${print.data.id}/billing-price`, {
    data: { amount: 60 },
  });
  expect(locked.status()).toBe(409);
  expect((await locked.json()).code).toBe("ITEM_LOCKED");
});
